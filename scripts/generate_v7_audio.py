#!/usr/bin/env python3
"""Generate v7 plan audio on Lyria and ElevenLabs and master beat-aligned loops.

Reads a v7 plan JSON (50 songs with bpm/meter/bars/structure/loopSeam), asks
...
29.000 s IMA4 CAF loops. Everything lands under scratch_data/music_v7/audio;
nothing touches the app bundle.

Identical conventions to generate_v6_audio.py, retargeted at scratch_data/music_v7.
"""
...
AUDIO_ROOT = PROJECT_ROOT / "scratch_data" / "music_v7" / "audio"
LOOP_ROOT = PROJECT_ROOT / "scratch_data" / "music_v7" / "loops"
MANIFEST_PATH = PROJECT_ROOT / "scratch_data" / "music_v7" / "audio_manifest.json"
...
def generate_one(
    ...
    with tempfile.TemporaryDirectory(prefix="rise-v7-audio-") as temporary:
        work = Path(temporary)
...
def main() -> int:
    ...
    if failures:
        log(f"v7 audio completed with {len(failures)} failure(s).")
        return 1
    log(f"SUCCESS: {len(jobs)} v7 sources generated and mastered under {AUDIO_ROOT}")
    return 0
