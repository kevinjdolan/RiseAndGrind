#!/usr/bin/env python3
"""Generate, master, and verify the authored v4 five-tier alarm music library.

Lyria masters land in scratch_data/TieredMusicMastersV4, generation evidence in
scratch_data/TieredLyriaResponsesV4, and mastered 29-second IMA4 CAF loops are
STAGED in scratch_data/TieredAppAudioV4. The currently shipped library under
RiseAndGrind/Resources/Sounds is not touched until every replacement validates;
run scripts/install_music_library_v4.py afterwards to swap the library in.
"""
...
MASTER_DIRECTORY = SCRATCH_DIRECTORY / "TieredMusicMastersV4"
RESPONSE_DIRECTORY = SCRATCH_DIRECTORY / "TieredLyriaResponsesV4"
STAGED_AUDIO_DIRECTORY = SCRATCH_DIRECTORY / "TieredAppAudioV4"
STAGED_MANIFEST_PATH = STAGED_AUDIO_DIRECTORY / "manifest.json"
...
def load_environment_key(name: str, env_file: Path = DEFAULT_ENV_FILE) -> str:
    """Load one secret from the environment or a KEY=VALUE env file, never logging it."""
...
def make_prompt(track: dict[str, Any]) -> str:
    """Build a v4 Lyria prompt for one original looping alarm song."""
    lyrics = "\n".join(f"- {line}" for line in track["performedLyrics"])
    language = track["language"]
    if language.lower() == "english":
        language_line = "LANGUAGE: sing in English."
    else:
        language_line = (
            f"LANGUAGE: {language}. Sing each lyric line exactly as written, "
            "keeping non-English lines in their original language with natural "
            "native pronunciation."
        )
    # The artist name stays out of the prompt: Google's input filter blocks
    # person-like names ("by the fictional act 'Mabel Dust'" → Input blocked),
    # and the vocalist/genre fields already carry everything audible.
    return (
        ...
        f'VOCALIST: {track["vocalistGender"]} lead vocal; {track["vocalistStyle"]}.\n'
        f"{language_line}\n"
    ...
    for attempt in range(1, max_attempts + 1):
        request = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "RiseAndGrind-TieredMusic/4.0",
            },
        )
        try:
...
def app_path(track: dict[str, Any]) -> Path:
    """Return the staged compact CAF path for one catalog track."""
    return STAGED_AUDIO_DIRECTORY / f"{track['filename']}.caf"
...
def outputs_are_valid(track: dict[str, Any]) -> bool:
    """Return whether one master, staged loop, and response record are complete."""
    try:
...
def generate_track(
    ...
    label = f"{track['intensityName']} {track['number']:03d} {track['displayName']}"
    ...
    log(f"START {label}")
    with tempfile.TemporaryDirectory(prefix="rise-v4-lyria-") as temporary:
...
def verify_catalog(catalog: tuple[dict[str, Any], ...]) -> list[dict[str, Any]]:
...
def finalize_catalog(catalog: tuple[dict[str, Any], ...]) -> None:
    """Write schema-4 staged manifest only after all 500 songs validate."""
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--only-tier",
        choices=[tier.identifier for tier in TIER_PROFILES_V4],
    )
...
def main() -> int:
    ...
    if arguments.verify_only:
        ...
        log(f"Verified and finalized all {TOTAL_SONGS} v4 tracks (staged).")
        return 0
    ...
    api_key = load_environment_key("GEMINI_API_KEY", arguments.env_file)
    ...
    STAGED_AUDIO_DIRECTORY.mkdir(parents=True, exist_ok=True)
    ...
    if all(outputs_are_valid(track) for track in catalog):
        ...
        log(
            f"SUCCESS: verified {TOTAL_SONGS} tracks in five tiers "
            f"({SONGS_PER_TIER} each); staged app audio "
