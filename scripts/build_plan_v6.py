#!/usr/bin/env python3
"""Merge, validate, and render the v6 song plan (10 songs per tier).

Reads scratch_data/music_v6/plan_vN/{tier}.json, runs the mechanical checks
...
def emit_yaml(songs: list[dict], catalog: str = "music_v6") -> str:
    label = catalog.replace("music_", "")
    lines = [
        f"# Rise & Grind music plan {label} — 50 loop-first songs (10 per tier).",
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="scratch_data catalog directory holding the plan (default: music_v6)",
    )
    arguments = parser.parse_args()
    base = PROJECT_ROOT / "scratch_data" / arguments.catalog
    ...
    (base / f"{arguments.version}.yaml").write_text(
        emit_yaml(songs, arguments.catalog) + "\n", encoding="utf-8"
    )
